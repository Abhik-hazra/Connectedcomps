...
# Perform necessary data preprocessing, handle string columns using StringIndexer
...
numeric_columns = [col for col, dtype in df.dtypes if dtype != 'string' and col != 'frd']
assembler = VectorAssembler(
    inputCols=numeric_columns + [f"{col}_index" for col in string_columns],
    outputCol="features"
)

# Chi-Square feature selection
selector = ChiSqSelector(numTopFeatures=5, featuresCol="features",
                         outputCol="selectedFeatures", labelCol="frd")

# Define RandomForestClassifier
rf = RandomForestClassifier(labelCol="frd", featuresCol="selectedFeatures", numTrees=100)

# Pipeline for preprocessing
pipeline = Pipeline(stages=indexers + [assembler, selector, rf])
